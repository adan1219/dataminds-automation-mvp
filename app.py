...
from utils.etl import ETLError, clean_data, compute_kpis, prepare_heatmap
from utils.pdf_report import create_report
from utils.plots import bar_top, heatmap, monthly_line, pareto
...
def process_file(file_bytes: bytes, file_name: str, file_hash: str):
    _ = file_hash  # usado para cachear por contenido
    bio = io.BytesIO(file_bytes)
    bio.name = file_name
    try:
        df, errors, warnings = clean_data(bio)
    except ETLError as exc:
        return pd.DataFrame(), [str(exc)], [], {}
    except Exception:
        return pd.DataFrame(), ["Ocurrió un error procesando el archivo. Verifica formato y contenido."], [], {}

    if errors:
        return pd.DataFrame(), errors, warnings, {}

    return df, errors, warnings, compute_kpis(df)
...
if not uploaded:
    ...
    df, errs, warns, kpis = process_file(file_bytes, file_name, file_hash)

    st.caption(f"Archivo cargado correctamente | Hash: {file_hash[:12]}…")
    ...
    for key, show in selections.items():
        if show:
            ...
            render_cols[slot % 3].metric(label, formatter(kpis.get(key)))
            slot += 1
    ...
    if os.getenv("OPENAI_API_KEY"):
        try:
            ...
            st.warning(f"No se pudo generar el resumen con IA: {exc}")
    ...
    if st.button("Generar PDF"):
        pdf_bytes = create_report(df, kpis, acciones, logo_path=None)
        st.download_button(
            "Descargar PDF",
            data=pdf_bytes,
